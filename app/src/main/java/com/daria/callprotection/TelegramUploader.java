...
/**
 * TelegramUploader — centralized uploader with error handling and background execution.
 * Uses Config.BOT_TOKEN and Config.CHAT_ID from project config.
 */
public class TelegramUploader {
    ...
    public interface Callback {
        void onComplete(boolean success, String info);
    }
    ...
    // Backwards-compatible names used in code (synchronous wrapper replaced with async)
    public static void sendText(final String text) {
        sendTextAsync(text, null);
    }

    public static void uploadFile(final Context ctx, final File file, final String caption) {
        uploadFileAsync(file, caption, null);
    }

    // ======= Internal sync helpers =======
    private static boolean sendMessageSync(String text) {
        if (!isNetworkAvailable()) {
            FileLogger.log("[TG] sendMessage: no network");
            return false;
        }
        try {
            ...
            handleError("sendMessageSync", text, e);
            return false;
        }
    }
    ...
    private static boolean sendMultipartFileSync(File file, String caption) {
        if (file == null || !file.exists()) {
            FileLogger.log("[TG] sendMultipartFile: file missing: " + (file == null ? "null" : file.getAbsolutePath()));
            return false;
        }
        if (!isNetworkAvailable()) {
            FileLogger.log("[TG] sendMultipartFile: no network for " + file.getName());
            return false;
        }
        ...
        try {
            ...
            if (ok) {
                try {
                    ...
                    FileLogger.log("[TG] deleted file=" + file.getName() + " ok=" + deleted);
                } catch (Exception e) {
                    FileLogger.log("[TG] delete failed: " + e.getMessage());
                }
            }
            conn.disconnect();
            return ok;
        } catch (Exception e) {
            handleError("sendMultipartFileSync", file.getName(), e);
            return false;
        }
    }
